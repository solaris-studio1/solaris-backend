...
router.post('/register', async (req, res) => {
  try {
    ...
    res.status(201).json({ token, user: { id, email, username, full_name } });
  } catch (err) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({ token, user: { id: user.id, email: user.email, username: user.username, full_name: user.full_name } });
  } catch (err) {
...
router.get('/me', async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT id, email, username, full_name, vip_level, kyc_status, created_at FROM users WHERE id = $1'